...
`.claude/skills/` is a project-local mirror of the same links, committed to the repo: each entry is a relative symlink `.claude/skills/<name>` → `../../skills/<bucket>/<name>`, covering every skill except `deprecated/` (matching `link-skills.sh`'s exclusion). It lets Claude Code auto-discover this repo's skills when working directly in the repo, without needing `scripts/link-skills.sh` run against the user's home directory first. Add, remove, or re-target the matching symlink whenever a skill is added, removed, or renamed.
